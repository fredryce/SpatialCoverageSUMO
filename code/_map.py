	...
	def parse_map(self): #new parse map for grid london
		...
		for item in edge_list:
			...
			try:
				...
				self.edges[item.attributes['id'].value] = Edge(item.attributes['from'].value, item.attributes['to'].value,30 , self.calculate_distance(item.attributes['from'].value, item.attributes['to'].value), std=5, grid=False)
