...
logging.basicConfig(filename=os.path.join("./", 'output.log'), filemode='w', format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', level=logging.INFO)
...
def changeRouteTest(route_dict, simnumber, vis = False, to_csv=False,heatmap_dict=None, spatial_matrix=None, global_heatheap=None,save_dir=None):
	...
	for routeobj in route_dict:
		...
		print(f"Route {routeobj.id} from {from_node} to {to_node} dist is {base_distance} expected dev of {dev_value} sp:{routeobj.sp} to {choice_junction} actual dev {dev_value - potential_dict[choice_junction]}")
	...
	for routeobj in route_dict:
		...
		print(f"Testing.... {routeobj.id} {dev_junction} {to_node}")
...
def find_non_div_route(route_dict, vis=False):
	...
	for routeobj in route_dict:
		...
		new_path_end = []
		route_to_target = env.sim_env.map_data.sumonet.find_shortest_path(from_node, dev_junction[routeobj.id])
		#print(f"{routeobj.id} {dev_junction} {to_node}")
		route_from_target = env.sim_env.map_data.sumonet.find_shortest_path(dev_junction[routeobj.id], to_node)[1:]
...
def get_route_dict(initial_routes, factor_change=None):
	points_total = []
	player_obj = {}
	for i, route in initial_routes.items():
...
def main_factor_test(const_routes=False):
	...
	for i in factor_values:
		while j < simulation_number:
			try:
				...
				print(f"Variable {i} simulation {j} base_cov:{coverage_value} algo_cov:{coverage_value_after} greedy_cov:{coverage_value_after_after}")
	...
	pd.DataFrame(pd_list,columns=["sim", "variable_value","playerNumber","baseCoverage", "ALGOCoverage","greedyCoverage"]).to_csv("coverage.csv")
...
if __name__ == "__main__":
	...
	#main()
	#main_test()
	main_factor_test(const_routes=True)
	traci.close()
